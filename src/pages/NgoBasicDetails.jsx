...
export default function NgoBasicInfo() {
  ...
  const handleSignup = async () => {
    if (!name.trim()) {
      alert("Enter NGO contact person name");
      return;
    }
    if (!email.trim()) {
      alert("Enter email");
  ...
  return (
    <div className="auth-wrapper">
      <div className="auth-card">
        <h2 className="auth-title">Complete NGO Signup</h2>
        ...
        <p className="back-link" onClick={() => navigate("/ngo-auth")}>
          Back to Login
        </p>
      </div>
    </div>
