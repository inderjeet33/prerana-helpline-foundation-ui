...
// const completionPercent = Math.round(Math.min(100, (raised / campaign.targetAmount) * 100));
...
export default function CampaignDetails() {
  ...
  // ✅ NEW: Withdraw Campaign
  async function withdrawCampaign() {
    const confirm = window.confirm(
      "Are you sure you want to withdraw this campaign? This will stop all future support."
    );
    ...
    const res = await fetch(`http://localhost:8080/campaigns/${campaign.id}/withdraw`, {
      method: "PATCH",
      headers: { Authorization: "Bearer " + token }
    });

    console.log("response is ",res);
    alert("Campaign has been withdrawn");
    loadCampaign();
  }
  ...
  const isOwner = campaign.ownerId === user?.id;
  const isClosed = campaign.status === "COMPLETED" || campaign.status === "WITHDRAWN";
